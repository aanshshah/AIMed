...
def warn(*args, **kwargs):
    pass import warnings warnings.warn = warn import os import numpy as np import pandas as pd from 
sklearn.model_selection import train_test_split, GridSearchCV,StratifiedKFold, cross_validate from 
sklearn.linear_model import LogisticRegression from sklearn.ensemble import RandomForestClassifier from 
sklearn.pipeline import Pipeline import csv from scipy import interp from tensorflow.keras.wrappers.scikit_learn 
import KerasClassifier from sklearn.metrics import classification_report, average_precision_score, 
precision_recall_curve, roc_curve, auc, precision_score, roc_curve, confusion_matrix, 
precision_recall_fscore_support, f1_score, precision_score, recall_score, accuracy_score from 
tensorflow.keras.losses import binary_crossentropy from tensorflow.keras.activations import softmax, relu from 
sklearn.preprocessing import StandardScaler from tensorflow.keras.models import Sequential from 
tensorflow.keras.layers import Dense, Dropout from tensorflow.keras.optimizers import Adam from 
sklearn.metrics.scorer import make_scorer import xgboost as xgb from xgboost.sklearn import XGBClassifier from 
tensorflow.keras.backend import clear_session import tensorflow from tensorflow.compat.v1 import ConfigProto from 
tensorflow.compat.v1 import InteractiveSession from tensorflow.keras.backend import set_session def 
classification_report_csv(y_test, y_preds, name, full_path, new_file=True):
    ...
    else: report_df.to_csv(open(full_path, 'a'), index=False, header=False) def store_cluster_info(y_preds, y_real, 
name, new_file=True):
    filename = 'results/neuralnet_alldata_results.csv'
    classification_report_csv(y_real, y_preds, name, filename, new_file) def preprocess():
    df = pd.read_csv('../data/x_with_lacefeatures.csv')
    df = df.drop(['subject_id', 'hadm_id'], axis=1)
    y = pd.read_csv('../data/y_more_no_df_clean.csv')
    return df, y def sensitivity(y_true, y_pred):
    true_positives = K.sum(K.round(K.clip(y_true * y_pred, 0, 1)))
    possible_positives = K.sum(K.round(K.clip(y_true, 0, 1)))
    return true_positives / (possible_positives + K.epsilon()) def specificity(y_true, y_pred):
    true_negatives = K.sum(K.round(K.clip((1-y_true) * (1-y_pred), 0, 1)))
    possible_negatives = K.sum(K.round(K.clip(1-y_true, 0, 1)))
    return true_negatives / (possible_negatives + K.epsilon()) def create_model(first_neuron=64, second_neuron=32, 
second_activation='relu',
    ...
    return model def create_pipeline_baseline():
    pipeline = []
    ...
    return pipeline def create_pipeline_nn():
    ...
    name = 'Neural_Network'
    store_cluster_info(y_pred, y_test, name, new_file=True) config = ConfigProto(allow_soft_placement=True) 
